...
Copyright (c) 2024, Spacemit Corporation. All rights reserved.
  ...
  )
{
  ...
  Status = SpiFlashProtocol->Init (SpiFlashProtocol, TRUE);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  switch (Flag) {
    case PROBE:
      // Probe spi bus
      Status = FlashProbe (SpiFlashProtocol);
      if (EFI_ERROR (Status)) {
        return SHELL_ABORTED;
      } else {
        return Status;
  ...
  if ((LengthStr == NULL) & LengthFlag) {
    ...
    ByteCount = ShellHexStrToUintn (LengthStr);
    if (ByteCount < 0) {
  ...
  switch (Flag) {
    case READ:
    case READ_FILE:
      Status = SpiFlashProtocol->Read (SpiFlashProtocol, Offset, ByteCount, Buffer);
      break;
    case ERASE:
    ...
    case WRITE:
    case WRITE_FILE:
      Status = SpiFlashProtocol->Write (SpiFlashProtocol, Offset, ByteCount, Buffer);
      break;
    case UPDATE:
    case UPDATE_FILE:
      Status = SpiFlashProtocol->Update (SpiFlashProtocol, Offset, ByteCount, Buffer);
      break;
  }
