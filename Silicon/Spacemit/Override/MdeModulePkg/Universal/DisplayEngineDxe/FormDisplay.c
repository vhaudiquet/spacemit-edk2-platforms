...
CHAR16  *mFrontPageConfirmChangeString;
CHAR16  *mFrontPageConfirmChangeOptionsString;
CHAR16  *mFrontPageConfirmExitString;
CHAR16  *mFrontPageConfirmExitOptionsString;
  ...
  )
{
  ...
  mFrontPageConfirmChangeString        = GetToken (STRING_TOKEN (FRONT_PAGE_CONFIRM_CHANGE), gHiiHandle);
  mFrontPageConfirmChangeOptionsString = GetToken (STRING_TOKEN (FRONT_PAGE_CONFIRM_CHANGE_OPTIONS), gHiiHandle);
  mFrontPageConfirmExitString          = GetToken (STRING_TOKEN (FRONT_PAGE_CONFIRM_EXIT), gHiiHandle);
  mFrontPageConfirmExitOptionsString   = GetToken (STRING_TOKEN (FRONT_PAGE_CONFIRM_EXIT_OPTIONS), gHiiHandle);
}
  ...
  )
{
  ...
  FreePool (mFrontPageConfirmChangeString);
  FreePool (mFrontPageConfirmChangeOptionsString);
  FreePool (mFrontPageConfirmExitString);
  FreePool (mFrontPageConfirmExitOptionsString);
  ...
  while (TRUE) {
    switch (ControlFlag) {
      ...
      case CfUiReset:
        ...
        //
        if (CompareGuid (&FormData->FormSetGuid, &gSpacemitFrontPageFormsetGuid)) {
          if (FormBrowserEx2) {
            CreateDialog (
              NULL,
              FormBrowserEx2->IsBrowserDataModified () ? \
              mFrontPageConfirmChangeString : \
              mFrontPageConfirmExitString,
              FormBrowserEx2->IsBrowserDataModified () ? \
              mFrontPageConfirmChangeOptionsString : \
              mFrontPageConfirmExitOptionsString,
              NULL
              );
            ...
            if (Key.ScanCode == SCAN_ESC) {
              ...
              ControlFlag        = CfRepaint;
              gUserInput->Action = BROWSER_ACTION_NONE;
            } else if ((Key.UnicodeChar | UPPER_LOWER_CASE_OFFSET) == (gConfirmOptYes[0] | UPPER_LOWER_CASE_OFFSET)) {
              ControlFlag        = CfExit;
              gUserInput->Action = FormBrowserEx2->IsBrowserDataModified () ? BROWSER_ACTION_SUBMIT | BROWSER_ACTION_RESET : BROWSER_ACTION_EXIT;
            } else {
              if (FormBrowserEx2->IsBrowserDataModified ()) {
                ControlFlag        = CfExit;
                gUserInput->Action = BROWSER_ACTION_EXIT;
              } else {
                Repaint            = TRUE;
                ControlFlag        = CfRepaint;
                gUserInput->Action = BROWSER_ACTION_NONE;
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    FormBrowserEx2->RegisterHotKey (&HotKey, BROWSER_ACTION_SUBMIT | BROWSER_ACTION_RESET, 0, NewString);
    FreePool (NewString);
