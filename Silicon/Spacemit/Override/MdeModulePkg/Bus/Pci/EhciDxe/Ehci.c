  ...
  )
{
  ...
  switch (PortFeature) {
    ...
    case EfiUsbPortPower:
      ...
      //
      // SpacemiT: Fix wrong Params, PPC belongs to HcStructParams instead of Cap
      if ((Ehc->HcStructParams & HCSP_PPC) == HCSP_PPC) {
        State |= PORTSC_POWER;
        EhcWriteOpReg (Ehc, Offset, State);
      }
  ...
  )
{
  ...
  Ehc->IsTDI = TRUE;
  DEBUG ((DEBUG_ERROR, "EhcDriverBindingStart: This EHC %s have TT for roothub extension\n", Ehc->IsTDI ? "does" : "does not\n"));
