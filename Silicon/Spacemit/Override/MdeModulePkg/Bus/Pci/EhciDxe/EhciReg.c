  ...
  )
{
  ...
  if (Ehc->IsTDI) {
    Val = EhcReadOpReg(Ehc, EHC_USBMODE_OFFSET);
    // If TDI EHC is not under host mode (by default),
    // it won't be at halt state after controller hard reset.
    if (!((Val & 3) == (3 << 0))) {
      Status = 0;
      goto DoReset;
    }
...
DoReset:
  Val = EhcReadOpReg(Ehc, EHC_USBCMD_OFFSET);
  EhcWriteOpReg(Ehc, EHC_USBCMD_OFFSET, (Val & ~USBCMD_RUN) | USBCMD_RESET);

  // Force set mode to Host mode.
  if (Ehc->IsTDI) {
    Val = EhcReadOpReg(Ehc, EHC_USBMODE_OFFSET);
    Val |= (3 << 0);
    Val &= ~(1 << 2);
    EhcWriteOpReg(Ehc, EHC_USBMODE_OFFSET, Val);
  }
  ...
  )
{
  ...
  //
  RegVal = EhcReadOpReg(Ehc, EHC_USBCMD_OFFSET);
  RegVal = ~(USBCMD_LRESET | USBCMD_IAAD | USBCMD_PSE | USBCMD_ASE | USBCMD_RESET);
  RegVal |= USBCMD_RUN;
  EhcWriteOpReg(Ehc, EHC_USBCMD_OFFSET, RegVal);
  //
