  ...
  )
{
  ...
  if (TRUE)
  {
    ...
    MapInfo->MappedHostAddress = PhysicalAddress;

	// NO need to rellocate buffer for EfiPciOperationBusMasterCommonBuffer
	if ((EfiPciOperationBusMasterCommonBuffer != Operation)
        && (EfiPciOperationBusMasterCommonBuffer64 != Operation)) {
      //
    ...
    MapInfo->Attributes = GcdDescriptor.Attributes;
    DEBUG ((
      DEBUG_VERBOSE,
      "Config PCI memory region [0x%Lx+0x%Lx] from 0x%x to uncache attributes\n",
      MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, MapInfo->Attributes
    ));
    mCpu->FlushDataCache (mCpu, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, EfiCpuFlushTypeInvalidate);
    Status = mCpu->SetMemoryAttributes (mCpu, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, EFI_MEMORY_UC);
    DEBUG ((
      DEBUG_VERBOSE,
      "%a, %d, %r: Set memory space uncached attributes for region [0x%Lx+0x%Lx]\n",
      __func__, __LINE__, Status, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE
    ));
  ...
  )
{
  ...
  DEBUG ((
    DEBUG_VERBOSE,
    "Restore PCI memory region [0x%Lx+0x%Lx] from uncache to 0x%x attributes\n",
    MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, MapInfo->Attributes
  ));
  Status = mCpu->SetMemoryAttributes (mCpu, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, MapInfo->Attributes);
  DEBUG ((
    DEBUG_VERBOSE,
    "%a: %d, %r, Reset memory space attribute for region [0x%Lx+0x%Lx]\n",
    __func__, __LINE__, Status, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE
  ));

  mCpu->FlushDataCache (mCpu, MapInfo->MappedHostAddress, MapInfo->NumberOfPages * EFI_PAGE_SIZE, EfiCpuFlushTypeInvalidate);

  // NO need to free buffer for EfiPciOperationBusMasterCommonBuffer
  if ((EfiPciOperationBusMasterCommonBuffer != MapInfo->Operation)
       && (EfiPciOperationBusMasterCommonBuffer64 != MapInfo->Operation)) {
    //
