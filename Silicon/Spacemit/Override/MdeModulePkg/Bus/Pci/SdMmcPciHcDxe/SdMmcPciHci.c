...
static const UINT8  TuningPattern4[] = {
...
static const UINT8  TuningPattern8[] = {
...
{
  UINTN        Index;
  CONST UINT8  *TuningPattern;
  UINT8        *TuningData = (UINT8 *)Packet->InDataBuffer;
  UINTN        PatternLen;

  if (Packet->InTransferLength == 128) {
    TuningPattern = TuningPattern8;
    PatternLen    = ARRAY_SIZE (TuningPattern8);
  } else {
    TuningPattern = TuningPattern4;
    PatternLen    = ARRAY_SIZE (TuningPattern4);
  }

  for (Index = 0; Index < PatternLen; Index++) {
    if (TuningData[Index] != TuningPattern[Index]) {
  ...
  )
{
  EFI_STATUS         Status;
  UINTN              Data;
  EMBEDDED_GPIO_PIN  CardDetectGpio;
  BOOLEAN            CardDetectActive;
  EMBEDDED_GPIO      *GpioProtocol;

  CardDetectGpio = PcdGet32 (PcdSdCardDetectGpioPin);
  if (CardDetectGpio == 0) {
    DEBUG ((DEBUG_ERROR, "CardDetectGpio not configured\n"));
    return RETURN_INVALID_PARAMETER;
  }

  CardDetectActive = PcdGetBool (PcdSdCardDetectActive);
  ...
  Status = GpioProtocol->Set (GpioProtocol, CardDetectGpio, GPIO_MODE_INPUT);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Set CardDetectGpio failed\n"));
    return Status;
  }

  Status = GpioProtocol->Get (GpioProtocol, CardDetectGpio, &Data);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Get CardDetectGpio failed\n"));
    return Status;
  }

  if ((Data == CardDetectActive)) {
    *MediaPresent = TRUE;
  } else {
    *MediaPresent = FALSE;
  }
  ...
  )
{
  ...
  Status = SdMmcHcRwMmio (PciIo, Slot, SD_MMC_HC_RX_DLINE_CTRL, FALSE, sizeof (DelayCode), &DelayCode);
  ...
  )
{
  ...
  if ((Packet->SdMmcCmdBlk->CommandIndex != EMMC_SEND_TUNING_BLOCK) &&
      (Packet->SdMmcCmdBlk->CommandIndex != SD_SEND_TUNING_BLOCK))
  {
  ...
  Status = SdMmcSoftwareReset (Private, Trb->Slot, ErrIntStatus);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  //
  Status = SdMmcCheckAndRecoverErrors (Private, Trb, IntStatus);
  if (EFI_ERROR (Status)) {
    goto Done;
  }
