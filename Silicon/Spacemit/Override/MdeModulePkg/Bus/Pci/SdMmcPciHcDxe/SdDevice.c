  ...
  )
{
  ...
  Status = SdMmcTuningPatternCheck (PassThru, Slot, &Packet);
  ...
  )
{
  ...
  Start = 0;
  while (Start <= 0xFF) {
    SdMmcRxSetDelaycode (PciIo, Slot, Start);
    Status = SdCardSendTuningBlk (PassThru, Slot);
    if (EFI_ERROR (Status)) {
      if (CurWindows) {
        DEBUG ((DEBUG_INFO, "Pass window [%d %d)\n", Min, Start));
      }
      ...
      CurWindows++;
      if (CurWindows > MaxWindows) {
        MaxWindows = CurWindows;
        Max        = Start;
      }
  ...
  if (MaxWindows < 50) {
    DEBUG ((DEBUG_ERROR, "Fail to find valid tuning window, MaxWindows:%d\n", MaxWindows));
    return EFI_INVALID_PARAMETER;
  }

  SdMmcRxSetDelaycode (PciIo, Slot, Max - MaxWindows/2);
  DEBUG ((DEBUG_INFO, "Set Rx Delaycode: %d\n", Max - MaxWindows/2));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  VoltageProtocol->EnableVoltage (
                                  VoltageProtocol,
                                  "aldo1"
                                  );

  VoltageProtocol->IsVoltageEnabled (
                                     VoltageProtocol,
                                     "aldo1",
                                     &Enabled
                                     );
  DEBUG ((DEBUG_INFO, "Get vqmmc voltage status: %d\n", Enabled));

  Status = VoltageProtocol->SetVoltage (
                                        VoltageProtocol,
                                        "aldo1",
                                        VoltageMicroVolt
                                        );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "SetVoltage failed\n"));
    return Status;
  }

  VoltageProtocol->GetVoltage (
                               VoltageProtocol,
                               "aldo1",
                               &VoltageMicroVolt
                               );
  DEBUG ((DEBUG_INFO, "Set vqmmc voltage: %d\n", VoltageMicroVolt));
  ...
  )
{
  ...
  if (PinCtrlProtocol->ApplyStateById == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: pinctrl state API is unavailable\n", __func__));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: failed to set pinctrl group Status=%r\n", __func__, Status));
    return Status;
  }
...
Voltage33Retry:
      ...
      ((Ocr & BIT24) != 0))
  {
    ...
    if (EFI_ERROR (Status)) {
      ...
      Status = SdMmcSetVoltage (PciIo, 1800000);
      if (EFI_ERROR (Status)) {
        Status = EFI_DEVICE_ERROR;
        goto Error;
      }
      ...
      Status = SdMmcHcSetPinctrl (PciIo, "uhs");
      if (EFI_ERROR (Status)) {
        Status = EFI_DEVICE_ERROR;
        goto Error;
      }
      ...
      // Enable autoclk
      OpExt  =  BIT11|BIT12;
      Status = SdMmcHcOrMmio (PciIo, Slot, SD_MMC_HC_OP_EXT_CTRL, sizeof (OpExt), &OpExt);
      if (EFI_ERROR (Status)) {
        return Status;
      }
  ...
  // Disable autoclk
  OpExt  = ~(BIT11|BIT12);
  Status = SdMmcHcAndMmio (PciIo, Slot, SD_MMC_HC_OP_EXT_CTRL, sizeof (OpExt), &OpExt);
  if (EFI_ERROR (Status)) {
    return Status;
  }
