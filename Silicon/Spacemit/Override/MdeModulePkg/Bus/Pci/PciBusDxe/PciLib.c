...
/**
  Determine whether should scan only for PCI Device 0.

  @param  Bridge           Bridge device instance.

  @retval TRUE if only Device 0 should be scanned for, otherwise FALSE.
**/
BOOLEAN
PciScanOnlyDevice0 (
  IN PCI_IO_DEVICE  *Bridge
...
{
  //
  // For Root Bridge, we should scan for all possible Devices.
  //
  ...
  // So scan only for Device 0.
  //
  if (Bridge->IsPciExp && (Bridge->PciExpDevicePortType == PCIE_DEVICE_PORT_TYPE_ROOT_PORT ||
                           Bridge->PciExpDevicePortType == PCIE_DEVICE_PORT_TYPE_DOWNSTREAM_PORT ||
                           Bridge->PciExpDevicePortType == PCIE_DEVICE_PORT_TYPE_PCI_TO_PCIE_BRIDGE)) {
    return TRUE;
  }
  ...
  )
{
  ...
  for (Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
    if (PciScanOnlyDevice0 (Bridge) && (Device > 0)) {
      break;
    }
