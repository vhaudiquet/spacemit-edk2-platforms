...
#define UNICODE_UPPER_Y                     AsciiCharToUpper ('y')
#define UNICODE_UPPER_N                     AsciiCharToUpper ('n')
...
/**
  This function is waiting for valid key pressed and returned Yes or No response.

  @retval  TRUE          Valid key is pressed and Yes response is returned.
  @retval  FALSE         Valid key is pressed and No response is restured.

**/
BOOLEAN
UiWaitForConfirmResponse (
  VOID
  )
{
  EFI_STATUS      Status;
  ...
  while (TRUE) {
    ...
    if (!EFI_ERROR (Status)) {
      if ((Key.ScanCode == SCAN_ESC) ||
          ((Key.UnicodeChar | UPPER_LOWER_CASE_OFFSET) == (UNICODE_UPPER_N | UPPER_LOWER_CASE_OFFSET)) ||
          ((Key.UnicodeChar | UPPER_LOWER_CASE_OFFSET) == (UNICODE_UPPER_Y | UPPER_LOWER_CASE_OFFSET))) {
        break;
      }
    }
  ...
  if ((Key.UnicodeChar | UPPER_LOWER_CASE_OFFSET) == (UNICODE_UPPER_Y | UPPER_LOWER_CASE_OFFSET)) {
    return TRUE;
  }
  ...
  )
{
  if ((QuestionId != FRONT_PAGE_KEY_CONTINUE)       &&
      ...
      (QuestionId != FRONT_PAGE_KEY_LOAD_DEFAULT)   &&
      (QuestionId != FRONT_PAGE_KEY_SAVE_CHANGE)    &&
      (QuestionId != FRONT_PAGE_KEY_DISCARD_CHAGNE) &&
      (QuestionId != FRONT_PAGE_KEY_SAVE_AND_EXIT) &&
      (QuestionId != FRONT_PAGE_KEY_DISCARD_AND_EXIT)) {
    return FALSE;
  }
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
    ...
    *Status = EFI_SUCCESS;
    switch (QuestionId) {
      ...
      case FRONT_PAGE_KEY_LOAD_DEFAULT:
        CreatePopUp (
          EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
          NULL,
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_LOAD_DEFAULT_MESSAGE), NULL),
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_CONFIRM_OPTIONS), NULL),
          NULL
          );

        if (UiWaitForConfirmResponse ()) {
          *Status = gFormBrowserEx2->ExecuteAction (BROWSER_ACTION_DEFAULT, EFI_HII_DEFAULT_CLASS_STANDARD);
        }
        break;

      case FRONT_PAGE_KEY_SAVE_CHANGE:
        CreatePopUp (
          EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
          NULL,
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_SAVE_CHANGES_MESSAGE), NULL),
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_CONFIRM_OPTIONS), NULL),
          NULL
          );

        if (UiWaitForConfirmResponse ()) {
          *Status = gFormBrowserEx2->ExecuteAction (BROWSER_ACTION_SUBMIT, 0);
        }
        break;

      case FRONT_PAGE_KEY_DISCARD_CHAGNE:
        CreatePopUp (
          EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
          NULL,
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_DISCARD_CHANGES_MESSAGE), NULL),
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_CONFIRM_OPTIONS), NULL),
          NULL
          );

        if (UiWaitForConfirmResponse ()) {
          *Status = gFormBrowserEx2->ExecuteAction (BROWSER_ACTION_DISCARD, 0);
        }
        break;

      case FRONT_PAGE_KEY_SAVE_AND_EXIT:
        CreatePopUp (
          EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
          NULL,
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_SAVE_AND_EXIT_MESSAGE), NULL),
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_CONFIRM_OPTIONS), NULL),
          NULL
          );
      ...
      case FRONT_PAGE_KEY_DISCARD_AND_EXIT:
        CreatePopUp (
          EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
          NULL,
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_DISCARD_AND_EXIT_MESSAGE), NULL),
          HiiGetString (HiiHandle, STRING_TOKEN (STR_POPUP_CONFIRM_OPTIONS), NULL),
          NULL
          );
  ...
  )
{
  ...
  if (gLanguageToken == NULL) {
    ...
    while (*LangCode != 0) {
      ...
      if (gLanguageToken[OptionCount] != ((EFI_STRING_ID) 0)) {
        OptionCount++;
      }
    }
  }
  ...
  )
{
  HiiCreateActionOpCode (
    ...
    STRING_TOKEN (STR_CONTINUE_HELP),
    EFI_IFR_FLAG_CALLBACK,
    0
    );
}
