...
/**
  Read a 32-bit DSI register

  @param  Offset  The offset of the DSI register to read.

  @return The value read from the DSI register.
**/
STATIC
inline
UINT32
...
{
  UINT32  Value = MmioRead32 (MIPI_REG_BASE + Offset);

  DEBUG ((DEBUG_VERBOSE, "DSI Read  [0x%08x] = 0x%08x\n", MIPI_REG_BASE + Offset, Value));
  return Value;
}
...
{
  DEBUG ((DEBUG_VERBOSE, "DSI Write [0x%08x] = 0x%08x\n", MIPI_REG_BASE + Offset, Data));
  MmioWrite32 (MIPI_REG_BASE + Offset, Data);
}
...
{
  DsiWrite (Offset, DsiRead (Offset) | Bits);
}
