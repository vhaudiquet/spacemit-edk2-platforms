  ...
  )
{
  ...
  Status = SdhciHcRwMmio (PciIo, BarIndex, Offset, TRUE, Count, &Data);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Data  |= Or;
  Status = SdhciHcRwMmio (PciIo, BarIndex, Offset, FALSE, Count, &Data);
...
/**
  Do AND/OR operation with the value of the specified SD/MMC host controller mmio register.
  ...
  @retval EFI_INVALID_PARAMETER The PciIo or AndData is NULL or the Count is not valid.
  @retval EFI_SUCCESS           The AND/OR operation succeeds.
  @retval Others                The AND/OR operation fails.
  ...
  )
{
  ...
  //
  Value  = 0;
  Status = SdhciHcRwMmio (PciIo, BarIndex, Offset, TRUE, Count, &Value);
  if (EFI_ERROR (Status)) {
    return Status;
  }
