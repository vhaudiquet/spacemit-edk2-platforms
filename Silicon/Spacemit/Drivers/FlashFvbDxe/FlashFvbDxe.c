...
#define SPI_FLASH_BASE_ADDR FixedPcdGet64 (PcdSFMemMapBaseAddress)

#define FW_ENV_REGION_BASE  FixedPcdGet64 (PcdFlashNvStorageVariableBase64)
...
FV_FLASH_INSTANCE   **mFlashInstances;
SPI_FLASH_PROTOCOL  *mFlash;

//
// Global variable declarations
//
FLASH_DESCRIPTION  mFlashDevices[] =
{
  {
    // UEFI Variable Services storage
    SPI_FLASH_BASE_ADDR,
    FW_ENV_REGION_BASE,
    FW_ENV_REGION_SIZE,
    SIZE_64KB,
    {
      0xc083c978, 0x91c9, 0x4ef9,{
        0x91, 0x6e, 0xc8, 0x41, 0x2b, 0x78, 0x48, 0x81
      }
    }
  }
};

CONST FV_FLASH_INSTANCE  mFlashInstanceTemplate =
{
  FV_FLASH_SIGNATURE, // Signature
  ...
  )
{
  ...
  if ((CompareGuid (&VariableStoreHeader->Signature, &gEfiVariableGuid) == FALSE)
      && (CompareGuid (&VariableStoreHeader->Signature, &gEfiAuthenticatedVariableGuid) == FALSE))
  {
  ...
  )
{
  ...
  StartAddress = GET_BLOCK_ADDRESS (
  ...
  Status = mFlash->Read (mFlash, (UINT32)ReadAddress, *NumBytes, Buffer);
  ...
  )
{
  ...
  Status = mFlash->Write (mFlash, (UINT32)WriteAddress, *NumBytes, (UINT8 *)Buffer);
  if (EFI_SUCCESS != Status) {
  ...
  )
{
  ...
  Status = MapRegToGcdRunTimeMmioSpace (Instance->DeviceBaseAddress, RuntimeMmioRegionSize);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  *Count        = ARRAY_SIZE (mFlashDevices);
...
FlashCreateInstance (
  ...
  OUT FV_FLASH_INSTANCE  **FlashInstance
  )
{
  EFI_STATUS         Status;
  FV_FLASH_INSTANCE  *Instance;
  ...
  Instance = AllocateRuntimeCopyPool (sizeof (FV_FLASH_INSTANCE), &mFlashInstanceTemplate);
  if (Instance == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  Status = mFlash->Write (mFlash, (UINT32)WriteAddress, BufferSizeInBytes, (UINT8 *)Buffer);
  if (EFI_SUCCESS != Status) {
    DEBUG ((DEBUG_ERROR, "%s - %d Status=%r\n", __FILE__, __LINE__, Status));
    return Status;
  }
  ...
  )
{
  ...
  Status = mFlash->Read (mFlash, (UINT32)ReadAddress, BufferSizeInBytes, Buffer);
  if (EFI_SUCCESS != Status) {
    DEBUG ((DEBUG_ERROR, "%s - %d Status=%r\n", __FILE__, __LINE__, Status));
    return Status;
  }
  ...
  )
{
  ...
  UINT32             I, FlashDeviceCount = 0;

  FlashPlatformGetDevices (&FlashDevices, &FlashDeviceCount);

  for (I = 0; I < FlashDeviceCount; I++) {
    EfiConvertPointer (0x0, (VOID **)&mFlashInstances[I]->DeviceBaseAddress);
    EfiConvertPointer (0x0, (VOID **)&mFlashInstances[I]->RegionBaseAddress);
    EfiConvertPointer (0x0, (VOID **)&mFlashInstances[I]);
  }

  EfiConvertPointer (0x0, (VOID **)&mFlash->Read);
  EfiConvertPointer (0x0, (VOID **)&mFlash->Write);
  EfiConvertPointer (0x0, (VOID **)&mFlash->Erase);
  ...
  )
{
  ...
  //
  mFlashInstances = AllocateRuntimePool ((UINT32)(sizeof (FV_FLASH_INSTANCE *) * FlashDeviceCount));
  if (mFlashInstances == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: Cannot allocate memory\n", __FUNCTION__));
    return EFI_OUT_OF_RESOURCES;
  }

  Status = gBS->LocateProtocol (&gSpacemitSpiFlashProtocolGuid, NULL, (VOID *)&mFlash);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[%a]:[%dL] Status=%r\n", __FUNCTION__, __LINE__, Status));
    FreePool (mFlashInstances);
    return Status;
  }

  Status = mFlash->Init (mFlash, TRUE);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[%a]:[%dL] Flash init fail %r\n", __FUNCTION__, __LINE__, Status));
    FreePool (mFlashInstances);
    return Status;
  }
  ...
  for (Index = 0; Index < FlashDeviceCount; Index++) {
    ...
    ContainVariableStorage =
      ...
      ((PcdGet64 (PcdFlashNvStorageVariableBase64) + PcdGet32 (PcdFlashNvStorageVariableSize))
       <= FlashDevices[Index].RegionBaseAddress + FlashDevices[Index].Size);
