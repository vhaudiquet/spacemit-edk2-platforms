  ...
  )
{
  ...
  MmcHostInstance->DiskInfo.Interface  = gEfiDiskInfoSdMmcInterfaceGuid;
  MmcHostInstance->DiskInfo.Inquiry    = MmcDiskInfoInquiry;
  MmcHostInstance->DiskInfo.Identify   = MmcDiskInfoIdentify;
  MmcHostInstance->DiskInfo.SenseData  = MmcDiskInfoSenseData;
  MmcHostInstance->DiskInfo.WhichIde   = MmcDiskInfoWhichIde;
  ...
  )
{
  ...
  if (MmcHostInstance->CardInfo.ExtCsd) {
  ...
  if (RemainingDevicePath != NULL) {
    ...
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      if ((Node.DevPath->Type != MESSAGING_DEVICE_PATH) ||
		  ((Node.DevPath->SubType != MSG_SD_DP) &&
		   (Node.DevPath->SubType != MSG_EMMC_DP)) ||
		  (DevicePathNodeLength (Node.DevPath) != sizeof (SD_DEVICE_PATH)) ||
		  (DevicePathNodeLength (Node.DevPath) != sizeof (EMMC_DEVICE_PATH)))
      {
  ...
  )
{
  ...
  CHAR16 ModelName[16];
  AsciiStrToUnicodeStrS ("eMMC", ModelName, 16);
  Status = AddUnicodeString2 (
    "eng",
    gMmcComponentName.SupportedLanguages,
    &MmcHostInstance->ControllerNameTable,
    ModelName,
    TRUE
    );
  ...
  Status = AddUnicodeString2 (
    "en",
    gMmcComponentName2.SupportedLanguages,
    &MmcHostInstance->ControllerNameTable,
    ModelName,
    FALSE
    );
  ...
  )
{
  ...
  while (CurrentLink != NULL && CurrentLink != &mMmcHostPool && (Status == EFI_SUCCESS)) {
    ...
    if (MmcHostInstance->ControllerNameTable != NULL) {
      FreeUnicodeStringTable (MmcHostInstance->ControllerNameTable);
    }
