...
*  Copyright (c) 2015, Spacemit Limited. All rights reserved.
*
...
#define EMMC_SWITCH_ERROR  BIT7

#define SD_BUS_WIDTH_1BIT  BIT0
#define SD_BUS_WIDTH_4BIT  BIT2

#define SD_CCC_SWITCH  BIT10
  ...
  )
{
  ...
  Cid   = (EMMC_CID *)MmcHostInstance->CardInfo.Cid;
  Csd   = (EMMC_CSD *)MmcHostInstance->CardInfo.Csd;
  ...
  Status = Host->ReceiveResponse (Host, MMC_RESPONSE_TYPE_CID, Response);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "EmmcIdentificationMode(): CID retrieval error, Status=%r.\n", Status));
    return Status;
  }

  CopyMem (((UINT8 *)Cid) + 1, Response, sizeof (EMMC_CID) - 1);
  PrintEmmcCid (Cid);
  ...
  Status = Host->ReceiveResponse (Host, MMC_RESPONSE_TYPE_CSD, Response);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "EmmcIdentificationMode(): CSD retrieval error, Status=%r.\n", Status));
    return Status;
  }

  CopyMem (((UINT8 *)Csd) + 1, Response, sizeof (EMMC_CSD) - 1);
  PrintEmmcCsd (Csd);
  ...
  // Fetch ECSD
  MmcHostInstance->CardInfo.ExtCsd = AllocatePages (EFI_SIZE_TO_PAGES (sizeof (EMMC_EXT_CSD)));
  if (MmcHostInstance->CardInfo.ExtCsd == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  Status = Host->SendCommand (Host, MMC_CMD8, CMD8_MMC_ARG);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "EmmcIdentificationMode(): ECSD fetch error, Status=%r.\n", Status));
  }

  Status = Host->ReadBlockData (Host, 0, 512, (UINT32 *)MmcHostInstance->CardInfo.ExtCsd);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "EmmcIdentificationMode(): ECSD read error, Status=%r.\n", Status));
    goto FreePageExit;
  }

  PrintEmmcExtCsd (MmcHostInstance->CardInfo.ExtCsd);
  ...
  Media->MediaId                       = ReadUnaligned32 ((UINT32 *)Cid->ProductSerialNumber);
  Media->ReadOnly                      = Csd->PermWriteProtect;
  Media->LogicalBlocksPerPhysicalBlock = 1;
  Media->IoAlign                       = 4;
  // Compute last block using bits [215:212] of the ECSD
  Media->LastBlock = ReadUnaligned32 ((UINT32 *)MmcHostInstance->CardInfo.ExtCsd->SecCount) - 1; // eMMC isn't supposed to report this for
...
FreePageExit:
  FreePages (MmcHostInstance->CardInfo.ExtCsd, EFI_SIZE_TO_PAGES (sizeof (EMMC_EXT_CSD)));
  return Status;
}
  ...
  )
{
  ...
  Host   = MmcHostInstance->MmcHost;
  ExtCsd = MmcHostInstance->CardInfo.ExtCsd;
  if (ExtCsd->DeviceType == EMMCBACKWARD) {
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  UINT32                 Buffer[128];
  UINT32                 Speed;
  UINTN                  BlockSize, CSizeMul, ReadBlLen;
  UINTN                  CardSize;
  UINTN                  NumBlocks;
  BOOLEAN                CccSwitch;
  SD_SCR                 Scr;
  ...
  Csd     = (SD_CSD *)MmcHostInstance->CardInfo.Csd;
  ...
  CopyMem (((UINT8 *)Csd) + 1, Response, sizeof (SD_CSD) - 1);
  PrintSdCsd (Csd);
  if (Csd->Ccc & SD_CCC_SWITCH) {
    CccSwitch = TRUE;
  } else {
    CccSwitch = FALSE;
  }

  if (0 == Csd->CsdStructure) {
    CardSize  = (Csd->CSizeHigh << 2 | Csd->CSizeLow) + 1;
    CSizeMul  = (1 << (Csd->CSizeMul + 2));
    ReadBlLen = (1 << (Csd->ReadBlLen));
    CardSize  = MultU64x32 (MultU64x32 ((UINT64)CardSize, CSizeMul), ReadBlLen);
  } else {
    Csd2     = (SD_CSD2 *)Csd;
    CardSize = (Csd2->CSizeHigh << 16 | Csd2->CSizeLow) + 1;
    CardSize = MultU64x32 ((UINT64)CardSize, SIZE_512KB);
  }

  BlockSize = 512;
  NumBlocks = CardSize / BlockSize;
  ...
  if (EFI_ERROR (Status)) {
    ...
    *((UINT32 *)&Scr + 1) = SwapBytes32 (Buffer[0]);
    if (Scr.SdSpec == 2) {
      if (Scr.SdSpec3 == 1) {
        if (Scr.SdSpec4 == 1) {
      ...
      if ((Scr.SdSpec3 == 0) && (Scr.SdSpec4 == 0)) {
        if (Scr.SdSpec == 1) {
  ...
  if (Scr.SdBusWidths & SD_BUS_WIDTH_4BIT) {
    CmdArg = MmcHostInstance->CardInfo.RCA << 16;
    Status = MmcHost->SendCommand (MmcHost, MMC_CMD55, CmdArg);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a (MMC_CMD55): Error and Status = %r\n", __func__, Status));
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    DEBUG ((DEBUG_INFO, "Card is SD2.0 => Supports high capacity\n"));
    IsHCS  = TRUE;
    ...
    if (!EFI_ERROR (Status)) {
      if (!MmcHostInstance->CardInfo.OCRData.PowerUp) {
        ...
        if ((MmcHostInstance->CardInfo.CardType == SD_CARD_2) && (MmcHostInstance->CardInfo.OCRData.AccessMode & BIT1)) {
          ...
          DEBUG ((DEBUG_INFO, "High capacity card.\n"));
        }
  ...
  Cid = (SD_CID *)MmcHostInstance->CardInfo.Cid;
  CopyMem (((UINT8 *)Cid) + 1, Response, sizeof (SD_CID) - 1);
  PrintSdCid (Cid);
