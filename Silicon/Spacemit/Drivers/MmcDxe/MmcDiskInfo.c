  ...
  )
{
  ...
  MmcHostInstance = MMC_HOST_INSTANCE_FROM_DISKINFO (This);

  if (*InquiryDataSize <= sizeof (MmcHostInstance->CardInfo.Cid)) {
    Status = EFI_SUCCESS;
    CopyMem (InquiryData, &MmcHostInstance->CardInfo.Cid, *InquiryDataSize);
  } else {
    Status = EFI_BUFFER_TOO_SMALL;
  }

  *InquiryDataSize = sizeof (MmcHostInstance->CardInfo.Cid);
