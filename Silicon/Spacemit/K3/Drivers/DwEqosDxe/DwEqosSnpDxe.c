...
)
{
  ...
  // Ensure the network interface is started
  if (Eqos->SnpMode.State == EfiSimpleNetworkStopped) {
    return EFI_NOT_STARTED;
  }
  ...
  // Validate HeaderSize against the mode's MediaHeaderSize
  if (HeaderSize != 0 && HeaderSize != Eqos->SnpMode.MediaHeaderSize) {
    DEBUG ((
      DEBUG_ERROR,
      "Header size mismatch: HeaderSize=0x%X, SnpMode.MediaHeaderSize=0x%X)\n",
      HeaderSize,
  ...
  if (BufferSize < Eqos->SnpMode.MediaHeaderSize) {
    DEBUG ((DEBUG_ERROR, "Buffer too small: BufferSize=0x%X\n", BufferSize));
    return EFI_BUFFER_TOO_SMALL;
  }

  if (BufferSize > EQOS_MAX_PACKET_SIZE) {
    DEBUG ((DEBUG_ERROR, "Buffer too large: BufferSize=0x%X\n", BufferSize));
  ...
  // Create the frame (if a header is present, prepare it)
  Frame = (UINT8 *)Buffer;
  ...
  // Now call the existing EqosSend function which handles the DMA, buffer, and descriptor
  Status = EqosSend(Eqos, Frame, BufferSize);
  if (EFI_ERROR (Status)) {
    goto Exit;
  }
...
)
{
  ...
  // Call EqosRecv2 to retrieve the packet (modified version)
  Status = EqosRecv(Eqos, Buffer, &Length);
  if (EFI_ERROR(Status)) {
    goto Exit;
  }

  // If the buffer size is too small, return an error and update the BufferSize
  ...
  *BufferSize = Length;
  ...
  // Optionally fill in the MAC addresses and protocol if requested
  if (DestAddr != NULL) {
    CopyMem(DestAddr, (EFI_MAC_ADDRESS *)Buffer, NET_ETHER_ADDR_LEN);
  }

  if (SrcAddr != NULL) {
    CopyMem(SrcAddr, (EFI_MAC_ADDRESS *)((UINT8 *)Buffer + 6), NET_ETHER_ADDR_LEN);
  }
  ...
  // Set the header size if requested
  if (HeaderSize != NULL) {
